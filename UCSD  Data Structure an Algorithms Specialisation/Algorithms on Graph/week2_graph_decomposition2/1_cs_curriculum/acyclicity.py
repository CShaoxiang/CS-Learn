...
def acyclic(adj):
    '''
    * Input: A directed graph (adjacency list).
    * Output: 1 if the graph contains a cycle, 0 otherwise.
    '''
    
    visited = [False] * len(adj)

    # Tracks nodes in the current DFS path (helps detect back edges, which indicate cycles)
    rec_visited = [False] * len(adj)
