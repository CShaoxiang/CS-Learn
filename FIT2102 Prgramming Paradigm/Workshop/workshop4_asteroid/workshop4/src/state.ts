...
export { initialState, reduceState, Rotate, Shoot, Tick, Thrust ,Teleport}
...
/**
 * Rotate the ship by adding a torque 
 */
class Rotate implements Action { 
    constructor(public readonly torque: number) { } 
    /**
     ...
     */
    apply = (s:State):State => ({
        ...s, ship: { ...s.ship, torque: this.torque }
    })
}
...
class Teleport implements Action { 
    ...
    apply = (s: State):State => ({ ...s,
        ship: { ...s.ship,
            ...
            acc:  Vec.unitVecInDirection(s.ship.angle)
                              .scale(Constants.teleport) 
...
class Tick implements Action {
    ...
    static handleCollisions = (s: State): State => {
        ...
        return {
            ...
            score : s.score + collidedBullets.length 
        }
    }
}
